...
namespace Map
{
    public class MapView : MonoBehaviour
    {
        ...
        public virtual void ShowMap(Map m)
        {
            ...
            ClearMap();// clear the map before showing a new one

            CreateMapParent();

            CreateNodes(m.nodes);//用map的node数据生成mapnode数据
        ...
        protected virtual void CreateMapParent()//创建地图父物体
        {
            firstParent = new GameObject("OuterMapParent");
            mapParent = new GameObject("MapParentWithAScroll");
            mapParent.transform.SetParent(firstParent.transform);
            var scrollNonUi = mapParent.AddComponent<ScrollNonUI>();
            scrollNonUi.freezeX = orientation == MapOrientation.BottomToTop || orientation == MapOrientation.TopToBottom;//冻结x轴
            scrollNonUi.freezeY = orientation == MapOrientation.LeftToRight || orientation == MapOrientation.RightToLeft;//冻结y轴
        ...
        [Tooltip("基于当前地图信息设置可以玩家可以到达或者不可以到达")]
